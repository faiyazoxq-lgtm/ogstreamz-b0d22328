DROP FUNCTION IF EXISTS public.list_nav_portals();

CREATE OR REPLACE FUNCTION public.list_nav_portals()
 RETURNS TABLE(id uuid, slug text, name text, kind text, vip boolean, by_boss boolean, paid boolean, created_at timestamp with time zone)
 LANGUAGE sql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
  SELECT
    p.id, p.slug, p.name, p.kind, p.vip,
